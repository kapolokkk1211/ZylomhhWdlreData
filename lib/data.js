...
const skillName = (s, lang) => (lang === 'th' ? s.th || s.cn : s.en || s.cn);
const skill = (s, lang) => ({
  name: skillName(s, lang),
  cn: s.cn,
  max: s.max || null,
  up: s.up ? { name: skillName(s.up, lang), cn: s.up.cn, max: s.up.max || null } : null,
  sp: s.sp ?? null,
  way: s.way || null,
...
export const companionRows = (lang = 'en') =>
  companions.map((c) => ({
    id: c.id,
    name: nm(c.name, lang),
    alt: [c.name.cn, lang === 'th' ? c.name.en : c.name.th].filter(Boolean).join(' · '),
    cn: c.name.cn,
    ...
    rebirthSkill: c.rebirthSkill,
    rebirthSkillName: lang === 'th' ? c.rebirthSkillTh || null : null,
    floors: c.floors,
...
export const companionSources = () => sourceLinks.companions;

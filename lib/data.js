...
import rankclimb from '@/content/data/rankclimb.json';
import advancement from '@/content/data/advancement.json';
...
/* The Bahamut threads we have translated outright, listed on the about page so a reader can
   check the original rather than take our Thai for it. */
export const threadSources = (lang) =>
  sourceLinks.threads.map((s) => ({
    id: s.id,
    url: s.url,
    title: pick(s.title, lang),
    used: pick(s.used, lang),
  }));
...
/* ---- การไต่แรงค์รวมของ page: a translated guide, both languages ship. ---- */
export const rankClimbData = () => rankclimb;

/* ---- จุติ 1 / จุติ 2 sections on the ตัวละครหลัก page ---- */
export const advancementData = () => advancement;
